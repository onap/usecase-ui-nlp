# coding=utf-8
# squad 接口
# 需要参数： FLAGS_output_dir ：训练时表示模型训练输出路径，输出训练得到的模型等；预测时表示模型预测结果的输出路径
#          FLAGS_init_checkpoint_squad ： 模型初始化路径，如果是训练就用bert预训练模型，如果是预测就用训练时候的输出路径
#          FLAGS_predict_file ： 待预测文件，csv文件
#          FLAGS_train_file : 待训练文件，csv文件
#          FLAGS_do_predict : 是否是预测
#          FLAGS_do_train ： 是否是训练
#          FLAGS_train_batch_size ：训练时的batch_size， 默认为 16
#          FLAGS_predict_batch_size : 预测时的batch_size，默认为 8
#          FLAGS_learning_rate ： 训练时的learning_rate ， 默认为5e-5
#          FLAGS_num_train_epochs ： 训练时的epochs， 默认为 3
#          FLAGS_max_answer_length ： 问题答案的最长长度 ， 默认为100个字符
#          FLAGS_max_query_length ： 问题的最长长度，默认64
#          FLAGS_version_2_with_negative ： 是否存在没有答案的问题，默认 false，推理时务必设置为 False
...
def make_json(input_file,questions):
    ...
    for i in data_train.index:
        data_inside['title'] = '暂无'
        data_inside['paragraphs'] = []
        paragraphs_inside = {}
        paragraphs_inside['context'] = data_train.loc[i,'文本']
