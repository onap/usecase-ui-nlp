    ...
    def __init__(self, port=5000, workers=4):
        ...
        def text_analyse():
            if not request.json or not 'task_id' in request.json:
                abort(400)
            if check_threads():
                return jsonify({"Des": "任务列表已满，无法提交新任务！", "Result": "训练任务提交失败", "Status": "ERROR"})
                ...
                try:
                    ...
                    return jsonify({"message": "任务提交成功", "status": "0"})
        ...
        def todo_status():
            ...
            try:
                if task is None:
                    return jsonify({'Des': '未找到该任务', 'Status': 'ERROR'})
                else:
                    if task.done():
                        print(task.result)
                        if task.result() == 'success':
                            ...
                            return jsonify({'Des': '程序执行错误，请查看执行日志', 'Status': 'ERROR'})
