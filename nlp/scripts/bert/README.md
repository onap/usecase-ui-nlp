...
python run_classifier.py \
  ...
  --init_checkpoint=$BERT_BASE_DIR/bert.ckpt \
  --max_seq_length=128 \
  --train_batch_size=32 \
  --learning_rate=2e-5 \
...
```shell
python $SQUAD_DIR/evaluate-v1.1.py $SQUAD_DIR/dev.json ./squad/predictions.json
```
...
python run_squad.py \
  ...
  --init_checkpoint=$BERT_LARGE_DIR/bert.ckpt \
  --do_train=True \
  --train_file=$SQUAD_DIR/train.json \
  --do_predict=True \
  --predict_file=$SQUAD_DIR/dev.json \
